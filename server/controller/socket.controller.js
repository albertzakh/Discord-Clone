...
export default class SocketController {
  ...
  addMessage = async ({ message, userId, id, room }) => {
    ...
    await chat.updateOne({$push: {messages: {username:user.username, profilePic: user.profilePic, text:message, createdAt: `${Day} at ${HoursFormat}:${MinutesFormat} ${TimeFormat}`}}});
    ...
    this.socket.to(room).emit("recieve_message", messageSend);
  }
  ...
  fetchAllMessages = async ({ userId, id }) => {
      ...
      this.socket.emit("recieve_messages_all", {messages:chat.messages, friendName: friend.username});
  }
  ...
  createGroup = async ({ userId, groupMembers, title }) => {
      ...
      const chatGroup = await Group.create({name: title, members: [user._id, ...memberIds], chats: []});
  ...
  sendGroupMessage = async ({ message, userId, id, room }) => {
      if(!userId) console.log("NO USER");
      ...
      await group.updateOne({$push: {chats: {username:user.username, profilePic: user.profilePic, text:message, createdAt: `${Day} at ${HoursFormat}:${MinutesFormat} ${TimeFormat}`}}});

      this.socket.to(room).emit("recieve_group_message", messageSend);
  }
  ...
  fetchGroupMessages = async ({ userId, id }) => {
    if(!userId) console.log("No user");
      ...
      this.socket.emit("recieve_group_messages_all", {messages:group.chats, groupName: group.name});
  }
